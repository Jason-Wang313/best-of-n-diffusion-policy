...
\midrule
Claim audit & \path{results/claims_status.json}; \path{results/ideal_metrics_status.json}; \path{results/claims_status.md}; \path{results/ideal_metrics_status.md} & none \\
Controlled sampler & \path{controlled_sampler_curves.csv}; \path{controlled_sampler_effect_cis.csv}; \path{controlled_sampler_diversity.csv}; \path{controlled_sampler_summary.json} & \path{controlled_sampler_curves.png} \\
Controller & \path{audit_then_sample_decisions.csv}; \path{audit_then_sample_calibration.csv}; \path{audit_then_sample_summary.json} & \path{audit_then_sample_decision_regions.png} \\
Scorers and repair & \path{scorer_comparison_curves.csv}; \path{scorer_comparison_effect_cis.csv}; \path{calibration_repair_map.csv}; \path{scorer_comparison_summary.json} & \path{scorer_comparison.png} \\
Latency & \path{nk_budget_phase.csv}; \path{nk_budget_latency_effect_ci.csv}; \path{true_diffusion_runtime.csv}; \path{pusht_runtime.csv} & \path{nk_budget_phase_diagram.png}; \path{true_diffusion_runtime.png} \\
Learned-lite & \path{learned_policy_lite_training.csv}; \path{learned_policy_lite_effect_cis.csv}; \path{learned_policy_lite_receding_horizon.csv}; \path{learned_policy_lite_summary.json} & \path{learned_policy_lite_ood.png}; \path{toy_image_observations.png} \\
True DDPM/DDIM & \path{true_diffusion_curves.csv}; \path{true_diffusion_effect_cis.csv}; \path{true_diffusion_sampler_comparison.csv}; \path{true_diffusion_summary.json} & \path{true_diffusion_survival.png}; \path{true_diffusion_sampler_comparison.png} \\
PushT & \path{pusht_curves.csv}; \path{pusht_rollouts.csv}; \path{pusht_rollout_metric_effect_cis.csv}; \path{pusht_rollout_metric_seed_aggregate.csv}; \path{pusht_summary.json} & \path{pusht_best_of_n.png} \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
1 & Finite tie-aware \bon{} law is implemented and tested. & Supported \\
...
9--11 & Differentiation from adjacent projects and artifact-backed claim discipline. & Supported \\
12 & Learned diffusion-policy-lite validity checklist and multi-seed state/image evaluation. & Supported \\
13 & True epsilon-prediction action DDPM/DDIM effects survive with one-step and clean-target ablations. & Supported \\
14--15 & PushT simulator reranking law, rollout coverage, and success curves over actual simulator rollouts. & Supported \\
16--18 & Runtime-backed recommendations, global wording gated on true-DDPM plus PushT, and reviewer-skepticism checklist. & Supported \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
High $N$ helps everywhere. & Misaligned controlled, true-diffusion, and PushT scorers expose harmful high-$N$ selection. & \path{results/claims_status.md}, Claims 3, 13, 14 \\
...
This is only a toy sampler. & Global wording requires true-DDPM and PushT rollout-metric gates; learned-lite and controlled tiers are supporting context. & \path{ideal_metrics_status.json}, Claims 13--18 \\
Runtime is ignored. & Budget sweeps and measured true-diffusion/PushT runtimes are audited. & \path{nk_budget_latency_effect_ci.csv}; runtime CSVs \\
...
Robot validity is overstated. & Scope excludes real robots, hardware safety, production visual policies, and full visual PushT validation. & \path{paper/limitations.md}; \path{ideal_metrics_status.json} \\
\bottomrule
\end{tabular}
\end{table}
...
rg -n "Best-of-N always help[s]" paper docs README.md
\end{verbatim}
The paper PDF is written to \texttt{paper/iclr/main.pdf}. The build wrapper compiles with the official ICLR 2026 style proxy and fails if the page label immediately before references exceeds nine pages.

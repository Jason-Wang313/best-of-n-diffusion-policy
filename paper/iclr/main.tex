...
\title{How Many Diffusion Trajectories Should a Robot Sample?\\
Inference-Time Selection Laws for Diffusion Policies}
...
\item We test the mechanism across controlled samplers, learned diffusion-policy-lite models, true action DDPM/DDIM sampling, and PushT simulator rollouts.
...
We evaluate four tiers, each with claim-audited artifacts. Controlled diffusion-like samplers isolate the mechanisms. Learned diffusion-policy-lite models train small state-conditioned and $32{\times}32$ image-conditioned denoisers over action horizons. The main learned diffusion tier trains true epsilon-prediction action DDPM/DDIM samplers, including fast DDIM, stochastic DDPM, one-step consistency-style, and clean-target ablations. The PushT tier uses \texttt{gym\_pusht/PushT-v0} with low-dimensional observations, heuristic demonstrations for CPU-feasible training, sampled action trajectories, and actual simulator rollout utility. PushT is simulator evidence for the inference-time law, not real-robot or full visual-policy validation.

\begin{table}[t]
\caption{Audited full-run evidence. Gains are high-minus-low selected utility unless noted. The \bon{} claim is conditional: aligned oracle or calibrated tails can help, while misaligned tails can hurt.}
\label{tab:main-evidence}
\centering
\small
\begin{tabular}{@{}p{0.22\linewidth}p{0.23\linewidth}p{0.22\linewidth}p{0.16\linewidth}@{}}
\toprule
...
\ats{} & 16 admissions; min utility/tail/latency \lcb{}s $0.833/0.802/0.827$ & false-admit rate $0.0$; 112 \texttt{block\_high\_N} actions & 256 decision rows \\
...
PushT simulator & utility gain $0.121$, \ci{} low $0.0576$; max-coverage gain $0.103$, \ci{} low $0.0381$ & misaligned change $-0.048$, \ci{} high $-0.031$; success gain $0.0$ & 12 seed/episode units \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{minipage}{0.49\linewidth}
\centering
\includegraphics[width=\linewidth]{figures/pusht_best_of_n.png}
\end{minipage}
...
Runtime & \path{nk_budget_latency_effect_ci.csv}; \path{true_diffusion_runtime.csv}; \path{pusht_runtime.csv} & \path{nk_budget_phase_diagram.png}; \path{true_diffusion_runtime.png} \\
True DDPM/DDIM and PushT & \path{true_diffusion_effect_cis.csv}; \path{pusht_rollout_metric_effect_cis.csv} & \path{true_diffusion_survival.png}; \path{pusht_best_of_n.png} \\
\bottomrule
\end{tabular}
\end{table}

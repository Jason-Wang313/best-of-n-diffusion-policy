...
17. A FetchPush-v4 Gymnasium Robotics/MuJoCo benchmark path reranks actual sampled action trajectories using simulator rollout utility and shows aligned gains, low-diversity saturation, anti-tail failure/gap behavior, and selected progress/success curves.
18. Runtime recommendations are backed by both abstract `N x K` budget sweeps and measured wall-clock runtime for true diffusion, PushT, and FetchPush sampling/rollouts.
19. Global diffusion-policy wording is promoted only when the true-DDPM, PushT, and FetchPush rollout-metric gates pass. Controller/fix wording is promoted only when the controller/fix gate passes. Toy-controlled and learned-lite tiers remain diagnostic and supporting context.
20. The reviewer-skepticism checklist must pass: true DDPM survives, PushT survives, FetchPush survives, controller/fix evidence is present, no real-robot or full visual-policy overclaim is present, runtime evidence is present, negative controls are present, and no full-run low-power warning is active.

## Non-Claims

This repository does not claim real-robot validation, universal Diffusion Policy improvement, production-scale visual policy quality, or a production deployment rule. The PushT and FetchPush paths are simulator evidence trained from heuristic demonstrations, not full visual Diffusion Policy benchmark suites. The calibration map is evidence for "repairs at least one held-out lower-bound regime," not evidence that calibration always repairs high-`N` selection. Audit-Then-Sample is an audited inference-time controller for these CPU regimes, not a hardware safety certificate, and it does not certify high `N` when audited utility is missing.
